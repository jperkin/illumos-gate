...
static uint32_t
cpuid_extract(uint32_t line, uint32_t mask, uint32_t shift)
{
...
cpuid_fill_onecache(arm_cpuid_t *cpd, int level, boolean_t icache)
{
	...
	cache->acc_sets = cpuid_extract(ccsidr, CCSIDR_NUMSETS_MASK,
	    CCSIDR_NUMSETS_SHIFT) + 1;
	cache->acc_assoc = cpuid_extract(ccsidr, CCSIDR_ASSOC_MASK,
	    CCSIDR_ASSOC_SHIFT) + 1;
	cache->acc_linesz = sizeof (uint32_t) << (cpuid_extract(ccsidr,
	    CCSIDR_LINESIZE_MASK, CCSIDR_LINESIZE_SHIFT) + 2);
	cache->acc_size = cache->acc_sets * cache->acc_assoc *
	    cache->acc_linesz;
}
...
cpuid_verify(void)
{
	...
	/* Support V7 and above.  Currently we do nothing with PXN or EAE */
	vmsa = cpuid_extract(cpd->ac_mmfr[0], ARM_CPUID_MMFR0_STATE0_MASK,
...
cpuid_valid_ident(uint32_t ident)
{
	...
	/*
	 * We don't support anything other than ARMv7.
	 */
...
cpuid_valid_fpident(uint32_t ident)
{
	...
	vfp = cpuid_extract(ident, ARM_CPUID_VFP_ARCH_MASK,
	    ARM_CPUID_VFP_ARCH_SHIFT);
	if (vfp < ARM_CPUID_VFP_ARCH_V3_V2BASE ||
	    vfp > ARM_CPUID_VFP_ARCH_V3_V3BASE) {
